(function (global) {
  ...
  function validate(message) {
    ...
    if (!NATIVE_ACTIONS.has(message.action)) {
      return { ok: false, error: `unknown native action: ${message.action || '<missing>'}` };
    }
    const version = message.protocolVersion ?? 0;
    if (!Number.isInteger(version) || version < 0 || version > CURRENT_VERSION) {
    ...
    if (message.action === 'engagement' && version >= 1) {
      if (!['poll', 'prediction'].includes(message.kind)) {
        return { ok: false, error: 'engagement kind must be poll or prediction' };
      }
      if (!['upsert', 'remove'].includes(message.lifecycle)) {
        return { ok: false, error: 'engagement lifecycle must be upsert or remove' };
      }
      if (!String(message.channel || '').trim()) {
        return { ok: false, error: 'engagement channel is required' };
      }
      if (message.lifecycle === 'upsert' && !String(message.title || '').trim()) {
