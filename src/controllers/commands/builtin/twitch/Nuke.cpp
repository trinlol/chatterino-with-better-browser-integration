...
//  - Uses this fork's sendMessage path (no separate Helix spam sender);
//    /spam and /pyramid send via the normal IRC path with duplicate
//    bypass handling kept from the original.
//  - Settings keys moved under /betterbrowser/.
...
void runSpamStep(const std::shared_ptr<SpamJob> &job)
{
    ...
    // This fork has no separate Helix spam sender; the normal send path
    // already applies duplicate-message bypass where needed.
                         ...
                         int minIntervalMs = MIN_SPAM_INTERVAL_MS)
{
    ...
    if (activeSpams().contains(key))
    {
        ...
        ctx.channel->addSystemMessage(
            QStringLiteral("%1 is already running in this channel. Use %2 stop first.")
            ...
            const bool sameUser =
                ...
                (!job->senderLogin.isEmpty() &&
                 message->loginName.compare(job->senderLogin,
                                            Qt::CaseInsensitive) == 0);
            if (!sameUser ||
                !job->normalizedMessages.contains(
    ...
    {
        ctx.channel->addSystemMessage(
            QStringLiteral("%1 %2 message%3 every %4 ms. Use %5 stop to cancel.")
                .arg(startVerb)
                .arg(job->total)
                .arg(job->total == 1 ? "" : "s")
                .arg(job->intervalMs)
...
QString sendPyramid(const CommandContext &ctx)
{
    ...
    {
        ctx.channel->addSystemMessage(
            "The top of that pyramid is too long. Use a shorter message or a lower height.");
        return "";
    }
    ...
    startChatMessageJob(ctx, buildPyramidMessages(message, cappedHeight),
                        ...
                        ctx.twitchChannel->hasHighRateLimit() ? MIN_SPAM_INTERVAL_MS
