...
$report = Join-Path $env:TEMP 'chatterino-reload-extension-report.txt'
...
# 1) Force-reload the two ghost ids so Edge reads their folders and purges stale registrations
$p1 = Start-Process -FilePath $edge -ArgumentList @(
  "--user-data-dir=$userData", '--profile-directory=Default',
  '--no-first-run','--no-default-browser-check',
  '--reload-extension=oenpbjpibkeomkimhpldpmabdblmipoa',
  '--reload-extension=boiehajcdnhbdmebpnbihmmdfafihlkl',
  'about:blank'
) -PassThru
"RELOAD_PID=$($p1.Id)" | Out-File $report -Append
Start-Sleep -Seconds 22
Get-Process msedge -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 3

$spRaw2 = Get-Content "$userData\Default\Secure Preferences" -Raw
"AFTER RELOAD ghosts:" | Out-File $report -Append
...
"bogfpdfo after: location=$($e.location) disable=$($e.disable_reasons -join ',')" | Out-File $report -Append
