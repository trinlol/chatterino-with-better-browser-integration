...
async function readState() {
  return page.evaluate(() => {
    ...
    const toolbarPts = document.querySelector(
    ...
    return {
      ...
      gapPx: gap,
      pointsLabel:
        toolbarPts?.innerText?.replace(/\s+/g, " ").trim().slice(0, 40) || null,
      rewardDialog: !!document.querySelector(
        'div[role="dialog"] .reward-center__content, div[role="dialog"]:has(.reward-center__content)'
      ),
...
try {
  ...
  await shot(
    "07-native-toolbar-fixed",
    page.locator("#chatterino-toolbar-slot")
  );

  const nativeBtn = page.locator(
