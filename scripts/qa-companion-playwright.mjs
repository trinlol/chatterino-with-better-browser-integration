...
async function readState(page) {
  return page.evaluate(() => {
    ...
    return {
      companionActive: document.documentElement.classList.contains(
      ...
      pointsReplica: !!document.querySelector(
        '#chatterino-toolbar-slot [data-test-selector="community-points-summary"]'
      ),
      ...
      domPoints: !!document.querySelector(
      ...
      rewardDialog: !!document.querySelector(
        'div[role="dialog"] .reward-center__content'
      ),
...
async function simulateChatterinoWipe(page) {
  return page.evaluate((html) => {
    const shell = document.querySelector(".chat-shell");
    if (!shell?.children[0])
      return { ok: false, reason: "no chat-shell child" };
...
try {
  ...
  const toolbarBox = await page
    .locator("#chatterino-toolbar-portal")
    .boundingBox()
    .catch(() => null);
  if (toolbarBox) {
    await page.screenshot({
  ...
  if (pointsVisible) {
    ...
    const dialog = page.locator(
      'div[role="dialog"] .reward-center__content, div[role="dialog"]:has-text("Power-ups")'
    );
  ...
  if (await pointsBtn.isVisible().catch(() => false)) {
    ...
    const dialogAfterWipe = page.locator(
      'div[role="dialog"] .reward-center__content, div[role="dialog"]:has-text("Power-ups")'
    );
    console.log(
      "Dialog after wipe click:",
      await dialogAfterWipe
        .first()
        .isVisible()
        .catch(() => false)
    );
  ...
  if (state.toolbarSlot) {
    ...
    const inViewport =
      x >= 0 &&
      y >= 0 &&
      x + w <= state.viewport.w &&
      y + h <= state.viewport.h;
  ...
  console.log(
    "Prediction replica visible:",
    !!state.predictionReplica?.visible
  );
